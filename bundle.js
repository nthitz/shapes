...
/***/ function(module, exports, __webpack_require__) {
	...
	var _lineSegmentsIntersect = __webpack_require__(3);

	var _lineSegmentsIntersect2 = _interopRequireDefault(_lineSegmentsIntersect);

	var _polygonsIntersect = __webpack_require__(4);

	var _polygonsIntersect2 = _interopRequireDefault(_polygonsIntersect);
	...
	var _shapes = __webpack_require__(16);

	var _shapes2 = _interopRequireDefault(_shapes);

	var _examples = __webpack_require__(17);
	...
	var exampleDropdown = document.getElementById('exampleSelect');
	var resetButton = document.getElementById('reset');
	var output = document.getElementById('canvas');
	var errorConsole = document.getElementById('errors');
	...
	init();
	function init() {
	  input.onkeydown = _lodash2.default.debounce(reset, 200);
	  ...
	  input.value = _examples2.default[0];
	  input.focus();
	  reset();
	}

	function appendOptions() {
	  _examples2.default.forEach(function (example) {
	...
	function compile(code) {
	  ...
	  try {
	    newProgram = _shapes2.default.parse(code);
	  } catch (exception) {
	    updateErrorMessage(exception);
	    return false;
	  }
	  if ((0, _deepEqual2.default)(newProgram, program)) {
	    return false;
	  }
	  program = newProgram;
	  return true;
	...
	function updateErrorMessage(error) {
	  ...
	  if (error === null) {
	    ...
	    var text = 'line: ' + error.location.start.line + ' offset ' + error.location.start.offset + ': ' + error;
	...
	function execute(force) {
	  ...
	  var halfSize = program.size / 2;
	  var twoPi = Math.PI * 2;

	  var programConstants = {};
	  if (program && program.angle && program.angle.influence && program.angle.influence === 'flowField') {
	  ...
	  function draw() {
	    rafId = requestAnimationFrame(draw);
	    var x = Math.random() * dim;
	    var y = Math.random() * dim;
	    var angle = 0;

	    assignAngle();

	    var pass = checkConditions();
	    if (!pass) {
	      return;
	    }
	      ...
	      if (program.angle) {
	        if (program.angle.influence) {
	          if (program.angle.influence === 'centerOrientation') {
	            ...
	            angle = Math.cos(x * programConstants.flowField.x) + Math.cos(y * programConstants.flowField.y);
	            addRandomJitter();
	          }
	    ...
	    function checkConditions() {
	      ...
	      if (program.shape.type === 'line') {
	        shape = [[x - Math.cos(angle) * halfSize, y - Math.sin(angle) * halfSize], [x + Math.cos(angle) * halfSize, y + Math.sin(angle) * halfSize]];
	      ...
	      if (program.shape.type === 'line') {
	        testFunction = _lineSegmentsIntersect2.default;
	      } else if (program.shape.type === 'polygon') {
	      ...
	      var hitsNone = true;
	      var searchBounds = [0, shapes.length];
	      if (typeof program.condition.last !== 'undefined') {
	        var searchStart = Math.max(0, shapes.length - program.condition.last);
	      }
	      for (var i = searchBounds[0]; i < searchBounds[1]; i++) {
	        var testShape = shapes[i];
	    ...
	    function drawShape() {
	      ...
	      var newShape = shapes[shapes.length - 1];
	...
	function polygonsIntersect(poly1, poly2) {
	  return (0, _polygonsIntersect2.default)(poly1, poly2).length !== 0;
	}

	function makePolygon(cx, cy, radius, theta, sides) {
	  var format = arguments.length <= 5 || arguments[5] === undefined ? 'object' : arguments[5];

	  var points = [];
	  for (var i = 0; i < sides; i++) {
	    var angle = Math.PI * 2 * (i / sides) + theta + Math.PI / 4;
	...
	function circlesIntersecting(circle1, circle2) {
	  return Math.pow(circle2.x - circle1.x, 2) + Math.pow(circle2.y - circle1.y, 2) < Math.pow(circle2.r + circle1.r, 2);
	}
...
/***/ function(module, exports, __webpack_require__) {
	...
	const PolygonArray = __webpack_require__(8);
	const Edge = __webpack_require__(6).Edge;
	const pointState = __webpack_require__(7).pointState;
...
/***/ function(module, exports, __webpack_require__) {
	...
	const Edge = __webpack_require__(6).Edge;
	const Point = __webpack_require__(7).Point;
	const pointState = __webpack_require__(7).pointState;
	const direction = {
	    backward: 0,
	    forward: 1
	...
	module.exports = function () {
	  ...
	  function peg$parse(input) {
	    var options = arguments.length > 1 ? arguments[1] : {},
	        ...
	        peg$c1 = function peg$c1(object, angle, condition) {
	      // console.log(object)
	      // console.log(angle)
	      object.angle = angle;
	      ...
	      return object;
	        ...
	        peg$c8 = function peg$c8(random, modifier) {
	      ...
	      return null;
	        ...
	        peg$c13 = function peg$c13(influence) {
	      return influence;
	    },
	      ...
	      return {
	        ...
	        peg$c41 = function peg$c41(size, shape) {
	      return {
	        size: size,
	        shape: shape
	      };
...
/***/ function(module, exports) {
	...
	module.exports = ['20px triangle angle influenced by flow field not intersecting', '20px line random angle influenced by flow field', '40px line random angle influenced by orientation to center only draw if not intersecting', '30px square random angle only draw if intersecting', '40px circle only draw if intersecting', '20px 3gon', '30px hexagon', '20px triangle random angle only draw if not intersecting', '20px square random angle', '20px line random angle', '20px line only draw if not intersecting or first', '40px line random angle only draw if intersecting', '30px square random angle only draw if intersecting last 10 or first', '20px line only draw if not intersecting last 50 or first', '20px line only draw if not intersecting last 50', '20px line random angle only draw if not intersecting last 50', '20px line random angle influenced by flow field only draw if not intersecting last 50'];
